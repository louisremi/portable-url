# portable-url [![npm package version](https://badgen.net/npm/v/portable-url)](https://npm.im/portable-url) [![bundle size](https://badgen.net/bundlephobia/minzip/portable-url)](https://bundlephobia.com/result?p=portable-url) [![github license](https://badgen.net/github/license/louisremi/portable-url)](https://github.com/louisremi/portable-url/blob/master/LICENSE)
...
When you use the `url` module, webpack bundles [`node-url`](https://github.com/defunctzombie/node-url) for the browser. You can alias webpack to use `portable-url` instead, saving around 7.5kB:
...
Rollup does not bundle shims for Node.js modules like `url` by default, but we can add `url` support via `portable-url` using aliases:
...
With this in place, `import url from 'url'` will use `portable-url` and keep your bundle small.
...
`portable-url` relies on the DOM [URL API](https://developer.mozilla.org/en-US/docs/Web/API/URL) to work. For older browsers that don't support the `URL` API, a [polyfill](https://www.npmjs.com/package/url-polyfill) is available.
